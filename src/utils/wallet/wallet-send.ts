...
export async function mnemonicToSeed(mnemonic: any) {
    const bip39 = await import('bip39');
...
const deriveImportsEncryptionKey = (seed: any) => bip32.fromSeed(Buffer.from(seed, 'hex')).derivePath("m/10016'/0")
    .privateKey;
...
function setUnlockedMnemonicAndSeed(mnemonic: string|null, seed: string|null, importsEncryptionKey: Buffer | null | undefined) {
...
export async function storeMnemonicAndSeed(mnemonic: string, seed: string, password: string) {
