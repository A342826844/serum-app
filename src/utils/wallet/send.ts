...
import { SelectedTokenAccounts, TokenAccount } from '@/types/account';
...
export function getSelectedTokenAccountForMint(
    ...
    const filtered = accounts.filter(
        ({ effectiveMint, pubkey }) => mint.equals(effectiveMint)
        && (!selectedPubKey
...
export async function settleFunds({
  ...
  quoteCurrencyAccount: TokenAccount;
    ...
    const signers = createAccountSigner
  ...
  selectedTokenAccounts?: SelectedTokenAccounts;
    ...
    const getOpenOrdersAccountsForProgramId = async (programId: PublicKey) => {
        ...
        return openOrdersAccounts.filter(
            (openOrders) => openOrders.baseTokenFree.toNumber()
...
export async function placeOrder({
  ...
  quoteCurrencyAccount: PublicKey | undefined;
    ...
    const formattedMinOrderSize = market?.minOrderSize?.toFixed(getDecimalCount(market.minOrderSize))
    || market?.minOrderSize;
    const formattedTickSize = market?.tickSize?.toFixed(getDecimalCount(market.tickSize))
    || market?.tickSize;
    const isIncrement = (num: number, step: number) => Math.abs((num / step) % 1) < 1e-5
    || Math.abs(((num / step) % 1) - 1) < 1e-5;
    if (isNaN(price)) {
        Notify({ message: 'Invalid price', type: 'danger' });
        return;
    }
    if (isNaN(size)) {
        Notify({ message: 'Invalid size', type: 'danger' });
        return;
    }
    if (!wallet || !wallet.publicKey) {
        Notify({ message: 'Connect wallet', type: 'danger' });
        return;
    }
    if (!market) {
        Notify({ message: 'Invalid  market', type: 'danger' });
        return;
    }
    ...
    if (size < market.minOrderSize) {
        Notify({ message: 'Size too small', type: 'danger' });
        return;
    }
    ...
    if (price < market.tickSize) {
        Notify({ message: 'Price under tick size', type: 'danger' });
        return;
    }
        ...
        (async () => {
            ...
            try {
                connection.onSignature(
                    txid,
                    (res) => {
                        console.log('WS confirmed', txid, res);
                        done = true;
                        if (res.err) {
                            reject(res.err);
                        } else {
            ...
            while (!done) {
                ...
                (async () => {
                    try {
                        ...
                        const res: any = signatureStatuses && signatureStatuses.value[0];
                        if (!done) {
                            if (!result) {
                                console.log('REST null result for', txid, res);
                            } else if (res.err) {
                                console.log('REST error for', txid, result);
                                done = true;
                                reject(res.err);
                            } else if (!res.confirmations) {
                                console.log('REST no confirmations for', txid, res);
                            } else {
                                console.log('REST confirmation for', txid, res);
                                done = true;
                                resolve(res);
                            }
                        }
